...
The **Min** and **Max** columns display the valid ranges of the variables, **Mode** lets you determine whether the variable is **global**, i.e. used for all jobs, or specific to a particular job. More about jobs below. Edited values in the panel are remembered at the next start. Changes are indicated with a white background. Exceeding the valid range is allowed, but indicated with a red background.
...
#define \_BACKTEST\_START 201501011ne \_B#define \_BACKTEST\_END   20251231
```
...
Naturally, training hundreds of jobs and variants will take its time. Enjoy a cup of tea inbetween.  
 
...
*   Make sure that **\_Lookback\_Bars** is sufficient for running all loaded jobs and the largest time frame. When extending or reducing the backtest years, adapt **\_WFO\_Cycles** accordingly.
...
*   For modifying **eval.c**, use a copy in the **Strategy** folder and include it with **#include "eval.c"**. Do not modify the original version in the **include** folder, since it will be overwritten on any update.
