...
*   The performance of the separate cycles is displayed in the [performance report](012_Performance_Report.md) under **Cycle performance**, the sample statistics under **Sample Cycles**. High performance differences between cycles normally indicates an unstable strategy. The global [statistics values](winloss.md) are the average over all sample cycles.
