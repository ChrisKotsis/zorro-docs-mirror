...
The stepwise change of variables and indicators can be visualized either with a [watch](166_watch.md) statement, or by displaying them in a [panel](142_panel.md) as with the [Evaluation Shell](010_Evaluation_Shell.md), or by [plotting](146_plot_plotBar.md) them in the chart. For debugging single loops or function calls, place **watch ("!...", ...)** statements inside. \[Step\] will then not proceed to the next bar, but to the next **watch** statement. Stepwise debugging normally begins at the end of the [LookBack](181_LookBack_UnstablePeriod.md) period. For beginning at a certain date or bar number, call [watch](143_printf_print_msg.md) dependent on a condition, f.i. **if(date() >= 20150401) watch("!...", ...);**. The [STEPWISE](018_TradeMode.md) flag is set during debugging; if you set it by script, the backtest starts already in debugging mode.
