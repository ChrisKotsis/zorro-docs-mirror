...
[DTN IQFeed](http://www.iqfeed.net)™ is a tick-by-tick unfiltered datafeed for almost all available symbols, with direct connection to the exchanges. The IQFeed plugin ([Zorro S](restrictions.md) required) was developed by Andrew Dolder. It connects to the IQFeed client application and can retrieve live and historical data. Unlike other plugins, the IQFeed plugin is not free. The annual subscription is **EUR 170**.

Using IQFeed:

*   Contact [oP group Support](mailto:support@opgroup.de) for subscribing the IQFeed plugin for your Zorro S license.
...
*   Download the IQFeed Client from the [DTN Download Page](https://www.iqfeed.net/index.cfm?displayaction=support&section=download) and install it into its default folder. Start one of the included apps to make sure that it works.
