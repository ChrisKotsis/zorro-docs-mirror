...
async function crawlPage(linkData, skipExisting = false) {
  ...
  if (skipExisting && await fs.pathExists(filePath)) {
    console.log(chalk.yellow(`⚡ Skipping (already exists): ${title}`));
    return {
      title,
      filename,
      level,
...
async function crawl() {
  console.log(chalk.bold('\n🕷️  Starting Zorro Documentation Crawler v3\n'));
  ...
  try {
    // Step 1: Extract all links from TOC (if not already done)
    if (!progress.tocLinks || progress.tocLinks.length === 0) {
      progress.tocLinks = await extractTOCLinks();
      await saveProgress(progress);
    }
    
    if (progress.tocLinks.length === 0) {
    ...
    for (let i = 0; i < progress.tocLinks.length; i++) {
      const linkData = progress.tocLinks[i];
      
      // Skip if already completed
      if (progress.completed.includes(linkData.url)) {
        pages.push({
          title: linkData.title,
          filename: `${String(linkData.index).padStart(3, '0')}_${sanitizeFilename(linkData.title)}.md`,
      ...
      const pageData = await limit(() => crawlPage(linkData));
      if (pageData) {
        pages.push(pageData);
        progress.completed.push(linkData.url);
        crawled++;
      } else {
        progress.failed.push(linkData.url);
      }
      ...
      if ((i + 1) % 10 === 0) {
        console.log(chalk.cyan(`Progress: ${i + 1}/${progress.tocLinks.length} pages processed`));
      }
    }
    ...
    console.log(chalk.bold.green(`\n✅ Crawling complete! ${progress.completed.length} pages processed.`));
    if (progress.failed.length > 0) {
      console.log(chalk.yellow(`⚠️  Failed to process ${progress.failed.length} pages`));
    }
